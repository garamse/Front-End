...
export default function Main({modalOpen}) {
  ...
  return (
    <main>
      <h2 className="ally">필요한 시간 알아보기</h2>
      <form className="cont-input" onSubmit={handleSubmit}>
        <p className="txt-wannabe">
          나는
          <input type="text" required value={expert} onChange={(event)=> setExpert(event.target.value)}/>
          전문가가 될 것이다.
        </p>
      ...
      {!!dday && <section className="cont-result">
        <h3 className="ally">결과 확인</h3>
        <p className="txt-wannabe">
          당신은 <strong>{expert}</strong> 전문가가 되기 위해서 <br />
          대략 <strong>{dday}</strong> 이상 훈련하셔야 합니다! :&#41; <br />
        </p>
